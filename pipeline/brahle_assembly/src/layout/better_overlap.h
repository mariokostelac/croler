...
class BetterOverlap {
 ...
 public:
  ...
  /**
   * Returns true for read_one if overlap looks like:
   * ------------      (read one)
   *      ------------ (read two)
   * Direction does not matter.
   */
  const bool GoesFrom(uint32_t read) const;

  /**
   * Returns true if overlap looks like:
   * ------------      (read one)
   *      ------------ (read two)
   * Direction does not matter.
   */
  const bool GoesFromFirst() const;
